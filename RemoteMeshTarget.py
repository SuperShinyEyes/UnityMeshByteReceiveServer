...
class RemoteMeshTarget(object):
    ...
    def read_int(self, socket: socket.socket):
        '''
        ...
        read_size = 1
        byte_end = socket.recv(read_size)
        byte_third = socket.recv(read_size)
        byte_second = socket.recv(read_size)
        byte_first = socket.recv(read_size)
        bytes = byte_first + byte_second + byte_third + byte_end
        print("bytes: {} len: {}".format(bytes, len(bytes)))
        int32, = unpack('i', bytes)
        return int32

    def start_recv_mesh(self, socket: socket.socket):
        '''
        :param socket:  
        :return: 
        1. get mesh size
        ...
        3. get mesh
        '''
        data_size = self.read_int(socket)
        data_buffer = b''
        print("Mesh size: {}".format(data_size))

        while len(data_buffer) < data_size:
            data_buffer += socket.recv(data_size - len(data_buffer))
